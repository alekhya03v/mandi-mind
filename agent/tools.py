...
def _cached_fetch(commodity, state, district=None, market=None):
    return AgmarknetClient().fetch(commodity, state, district, market)
...
def get_mandi_prices(commodity: str, state: str, district: str | None = None) -> dict:
    ...
    rows, source, note = _cached_fetch(commodity, state, district)
...
def compare_nearby_markets(commodity: str, state: str, top_n: int = 5) -> dict:
    ...
    rows, source, note = _cached_fetch(commodity, state)
...
@tool
def get_price_trend(commodity: str, state: str, market: str, days: int = 7) -> dict:
    """Describe recent modal-price movement for one commodity and market."""
    rows, source, note = _cached_fetch(commodity, state, market=market)
